...
def pval_from_corr_one_tail(r, dof):
    tstat = np.sqrt(dof / (1 - r * r)) * r
    return stats.t.sf(tstat, dof)
...
def df_cost_one_tail(r, dof):
    """minimize abs(1-alpha) as a function of M_eff"""
    pval = pval_from_corr_one_tail(r, dof)
    mean = np.mean(pval)
    var = np.var(pval)
    return mean * (mean * (1.0-mean) / var - 1.0) - 1.0
...
def fit_beta_parameters_one_tail(r_perm, dof_init, tol=1e-4, return_minp=False):
    """
    one-tail pvalue
      r_perm:    array of max. r values from permutations
      dof_init:   degrees of freedom
    """
    try:
        true_dof = scipy.optimize.newton(lambda x: df_cost_one_tail(r_perm, x), dof_init, tol=tol, maxiter=50)
    except:
        # print('WARNING: scipy.optimize.newton failed to converge (running scipy.optimize.minimize)')  # turn off
        res = scipy.optimize.minimize(lambda x: np.abs(df_cost_one_tail(r_perm, x)), dof_init, method='Nelder-Mead', tol=tol)
        true_dof = res.x[0]
    pval = pval_from_corr_one_tail(r_perm, true_dof)
    mean, var = np.mean(pval), np.var(pval)
    beta_shape1 = mean * (mean * (1 - mean) / var - 1)
    beta_shape2 = beta_shape1 * (1/mean - 1)
    res = scipy.optimize.minimize(lambda s: beta_log_likelihood(pval, s[0], s[1]), [beta_shape1, beta_shape2], method='Nelder-Mead', tol=tol)
...
def calculate_beta_approx_pval_one_tail(r_perm, r_nominal, dof_init, tol=1e-4):
    """
    one-tail pvalue
      r_nominal: nominal max. r (scalar or array)
      r_perm:    array of max. r values from permutations
      dof_init:   degrees of freedom
    """
    beta_shape1, beta_shape2, true_dof = fit_beta_parameters_one_tail(r_perm, dof_init, tol)
    pval_true_dof = pval_from_corr_one_tail(r_nominal, true_dof)
    pval_beta = stats.beta.cdf(pval_true_dof, beta_shape1, beta_shape2)
    return pval_beta, beta_shape1, beta_shape2, true_dof, pval_true_dof
