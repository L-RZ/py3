    ...
    for chrom in igc.chrs:
        ...
        if group_s is None:
            for k, (phenotype, genotypes, genotype_range, phenotype_id) in enumerate(igc.generate_data(chrom=chrom, verbose=verbose), k+1):
                ...
                # filter on phenotype to remove NA sample (mark as -1 or 'NA')
                not_na_phe = phenotype == -1
                genotypes = genotypes[not_na_phe]
                phe_covariates_df = covariates_df[not_na_phe]
                residualizer = Residualizer(torch.tensor(phe_covariates_df.values, dtype=torch.float32).to(device))
                dof = phenotype_df.shape[1] - 2 - phe_covariates_df.shape[1]
