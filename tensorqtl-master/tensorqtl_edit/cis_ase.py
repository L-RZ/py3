...
def calculate_cis_permutations_one_tail(genotypes_t, phenotype_t, permutation_ix_t, residualizer=None):
    """Calculate nominal and empirical correlations"""
    permutations_t = phenotype_t[permutation_ix_t]
    r_nominal_t, genotype_var_t, phenotype_var_t = calculate_corr(genotypes_t, phenotype_t.reshape(1,-1),
                                                                  residualizer=residualizer, return_var=True)
    ...
    std_ratio_t = std_ratio_t.squeeze(dim=-1)
    # corr_t = calculate_corr(genotypes_t, permutations_t, residualizer=residualizer).pow(2)  # genotypes x permutations
    corr_t = calculate_corr(genotypes_t, permutations_t, residualizer=residualizer)  # return r value instand of r2
    corr_t = corr_t[~torch.isnan(corr_t).any(1),:]
    if corr_t.shape[0] == 0:
        raise ValueError('All correlations resulted in NaN. Please check phenotype values.')
    # r2_perm_t,_ = corr_t.max(0)  # maximum correlation across permutations
    ...
    # ix = r2_nominal_t.argmax()
    # return r_nominal_t[ix], std_ratio_t[ix], ix, r2_perm_t, genotypes_t[ix]
    r_perm_t, _ = corr_t.max(0) # maximum correlation across permutations  base on r value
    r_nominal_t[torch.isnan(r_nominal_t)] = -1  # workaround for nanargmax()
    ix = r_nominal_t.argmax()
    return r_nominal_t[ix], std_ratio_t[ix], ix, r_perm_t, genotypes_t[ix]
        ...
        if group_s is None:
            for k, (phenotype, genotypes, genotype_range, phenotype_id) in enumerate(igc.generate_data(chrom=chrom, verbose=verbose), k+1):
                ...
                if covariates_df is not None:
                    ...
                    phe_covariates_t = covariates_t[phe_used_index, ]
                    residualizer = Residualizer(phe_covariates_t)
                    # dof = phenotype_df.shape[1] - 2 - phe_covariates_t.shape[1]
...
def prepare_cis_output(r_nominal, r_perm, std_ratio, g, num_var, dof, variant_id, tss_distance, phenotype_id, nperm=10000):
    ...
    pval_perm = (np.sum(r_perm >= r_nominal) + 1) / (nperm + 1)
    ...
    res_s = pd.Series(OrderedDict([
        ...
        ('pval_nominal', pval_from_corr_one_tail(r_nominal, dof)),  # one tail t-test p-value for ASE only
        ('slope', slope),
        ('slope_se', slope_se),
        ('pval_perm', pval_perm),
        ('pval_beta', np.NaN),
...
def map_cis(genotype_df, variant_df, phenotype_df, phenotype_pos_df, covariates_df=None,
    ...
    if group_s is None:
        for k, (phenotype, genotypes, genotype_range, phenotype_id) in enumerate(igc.generate_data(verbose=verbose), 1):
            ...
            # use r value instead of r2 for one-tail p-value t-test
            res = calculate_cis_permutations_one_tail(genotypes_t, phenotype_t, permutation_ix_t, residualizer=residualizer)
            r_nominal, std_ratio, var_ix, r_perm, g = [i.cpu().numpy() for i in res]
            var_ix = genotype_range[var_ix]
            variant_id = variant_df.index[var_ix]
            tss_distance = variant_df['pos'].values[var_ix] - igc.phenotype_tss[phenotype_id]
            res_s = prepare_cis_output(r_nominal, r_perm, std_ratio, g, genotypes_t.shape[0], dof, variant_id,
                                       tss_distance, phenotype_id, nperm=nperm)
            if beta_approx:
                # res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df',
                #        'pval_true_df']] = calculate_beta_approx_pval(r_perm, r_nominal, dof)
                res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df',
                       'pval_true_df']] = calculate_beta_approx_pval_one_tail(r_perm, r_nominal, dof)
            res_df.append(res_s)
    else:
    ...
    if group_s is None:
        for k, (phenotype, genotypes, genotype_range, phenotype_id) in enumerate(igc.generate_data(verbose=verbose), 1):
            ...
            while True:
                ...
                res = calculate_cis_permutations_one_tail(genotypes_t, phenotype_t, permutation_ix_t, residualizer=residualizer)
                r_nominal, std_ratio, var_ix, r_perm, g = [i.cpu().numpy() for i in res]
                x = calculate_beta_approx_pval_one_tail(r_perm, r_nominal, dof)
                ...
                if x[0] <= signif_threshold:
                    ...
                    res_s = prepare_cis_output(r_nominal, r_perm, std_ratio, g, genotypes.shape[0], dof, variant_id,
                                               tss_distance, phenotype_id, nperm=nperm)
            ...
            if forward_df.shape[0]>1:
                ...
                for k,i in enumerate(forward_df['variant_id'], 1):
                    ...
                    res = calculate_cis_permutations_one_tail(genotypes_t, phenotype_t, permutation_ix_t,
                    ...
                    if x[0] <= signif_threshold and variant_id not in variant_set:
                        tss_distance = variant_df['pos'].values[var_ix] - igc.phenotype_tss[phenotype_id]
                        # res_s = prepare_cis_output(r_nominal, r2_perm, std_ratio, g, genotypes.shape[0], dof, variant_id, tss_distance, phenotype_id, nperm=nperm)
                        res_s = prepare_cis_output(r_nominal, r_perm, std_ratio, g, genotypes.shape[0], dof, variant_id, tss_distance, phenotype_id, nperm=nperm)
                        res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df', 'pval_true_df']] = x
                        res_s['rank'] = k
