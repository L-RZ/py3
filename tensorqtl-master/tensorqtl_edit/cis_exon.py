...
def calculate_cis_nominal(genotypes_t, phenotype_t, residualizer=None):
    ...
    r_nominal_t, genotype_var_t, phenotype_var_t = calculate_corr(genotypes_t, p, residualizer=residualizer,
                                                                  return_var=True)
...
def map_nominal(genotype_df, variant_df, phenotype_df, phenotype_pos_df, prefix,
    ...
    igc = genotypeio.InputGeneratorCis(genotype_df, variant_df, phenotype_df, phenotype_pos_df, group_s=group_s,
                                       window=window)
    ...
    for chrom in igc.chrs:
        ...
        if group_s is None:
            for k, (phenotype, genotypes, genotype_range, phenotype_id) in enumerate(
                ...
                if interaction_s is None:
                    ...
                    if genotypes_t.shape[0] > 0:
                        ...
                        top_s = pd.Series(
                            [phenotype_id, variant_ids[ix], tss_distance[ix], maf[ix], ma_samples[ix], ma_count[ix],
            ...
            for k, (phenotypes, genotypes, genotype_range, phenotype_ids, group_id) in enumerate(
                    igc.generate_data(chrom=chrom, verbose=verbose), k + 1):
                ...
                if genotypes_t.shape[0] > 0:
                    ...
                    if interaction_s is not None:
                        ix = np.nanargmax(np.abs(tstat[:, 2]))
                        top_s = pd.Series(
                            [chr_res['phenotype_id'][start:start + n][ix], variant_ids[ix], tss_distance[ix], maf[ix],
                             ma_samples[ix], ma_count[ix],
                             tstat[ix, 0], b[ix, 0], b_se[ix, 0],
                             tstat[ix, 1], b[ix, 1], b_se[ix, 1],
                             tstat[ix, 2], b[ix, 2], b_se[ix, 2]], index=chr_res.keys())
        ...
        if run_eigenmt:
            if group_s is None:
                ...
                best_assoc['pval_emt'] = np.minimum(
                    best_assoc['num_phenotypes'] * best_assoc['tests_emt'] * best_assoc['pval_gi'], 1)
...
def prepare_cis_output(r_nominal, r2_perm, std_ratio, g, num_var, dof, variant_id, tss_distance, phenotype_id,
                       nperm=10000):
...
def _process_group_permutations(buf, variant_df, tss, dof, group_id, nperm=10000):
    ...
    res_s = prepare_cis_output(r_nominal, r2_perm, std_ratio, g, num_var, dof, variant_id, tss_distance, phenotype_id,
                               nperm=nperm)
    res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df', 'pval_true_df']] = calculate_beta_approx_pval(r2_perm,
                                                                                                               r_nominal * r_nominal,
                                                                                                               dof * 0.25)
...
def map_cis(genotype_df, variant_df, phenotype_df, phenotype_pos_df, covariates_df=None,
    ...
    if group_s is None:
        for k, (phenotype, genotypes, genotype_range, phenotype_id) in enumerate(igc.generate_data(verbose=verbose), 1):
            ...
            res_s = prepare_cis_output(r_nominal, r2_perm, std_ratio, g, genotypes_t.shape[0], dof, variant_id,
                                       tss_distance, phenotype_id, nperm=nperm)
            if beta_approx:
                res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df',
                       'pval_true_df']] = calculate_beta_approx_pval(r2_perm, r_nominal * r_nominal, dof)
            res_df.append(res_s)
    else:
        for k, (phenotypes, genotypes, genotype_range, phenotype_ids, group_id) in enumerate(
            ...
            res_s = _process_group_permutations(buf, variant_df, igc.phenotype_tss[phenotype_ids[0]], dof, group_id,
                                                nperm=nperm)
            res_df.append(res_s)
            ...
            if forward_df.shape[0] > 1:
                ...
                for k, i in enumerate(forward_df['variant_id'], 1):
                    ...
                    if x[0] <= signif_threshold and variant_id not in variant_set:
                        tss_distance = variant_df['pos'].values[var_ix] - igc.phenotype_tss[phenotype_id]
                        res_s = prepare_cis_output(r_nominal, r2_perm, std_ratio, g, genotypes.shape[0], dof,
                                                   variant_id, tss_distance, phenotype_id, nperm=nperm)
                        res_s[['pval_beta', 'beta_shape1', 'beta_shape2', 'true_df', 'pval_true_df']] = x
                        res_s['rank'] = k
