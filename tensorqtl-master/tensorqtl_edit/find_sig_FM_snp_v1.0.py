...
# find_sig_snp_v0.2
# v0.2.1 for exon ASE analysis. using rs_id to connect SNP
# v0.1 for exon ASE analysis from tensorQTL result. using python3
...
def proportion_p(junct_list, min_line_l, r2_dict, p_cutoff, r2_cutoff=0.4, snp_col_index=1, p_col_index=6):
    ...
    min_p_snpid = min_line_l[snp_col_index]
    min_p = min_line_l[p_col_index]

    if min_p_snpid not in r2_dict:
        print('ERROR: {} not in file.ld'.format(min_p_snpid))
        print(min_line_l)
        return
    ...
    for l in junct_list:
        l_l = l.strip().split('\t')
        l_snpid = l_l[snp_col_index]
        p = float(l_l[p_col_index])
...
def junct_region(p_addr, out_addr, r2_dict, p_cutoff=1e-9, r2_cutoff=0.4, snp_col_index=1, p_col_index=6):
    if p_addr.endswith('.gz'):
        f_p = gzip.open(p_addr)
    else:
        f_p = open(p_addr)
    fpath, fname = os.path.split(p_addr)
    name, ext = os.path.splitext(fname)
    if out_addr.endswith('.gz'):
        f_out = gzip.open(out_addr, 'w')
    else:
    ...
    #         f_out.write(header)
    #         break
    head_in = f_p.readline()
    # head_in_l = head_in.split('\t')
    header = head_in.strip() + '\tR2\tN_credible\tP_credible\tSNP_index\tP_index\tP_cred_index\n'
    f_out.write(header)
    ...
    for line in f_p:
        line_l = line.strip().split('\t')
        junct_id = line_l[0]
        if line_l[0] != 'phenotype_id' and line_l[p_col_index] != '':
            # print(line_l) # for testing
            line_p = float(line_l[p_col_index])
            if junct_id_old == '':
                ...
                if min_p < p_cutoff:  # run FM if minp of exon-skipping is < p_cutoff
                    out_line = proportion_p(junct_tmp_list, min_line_l, r2_dict, p_cutoff, r2_cutoff, snp_col_index, p_col_index)
                    if out_line:
...
def main():
    ...
    parser.add_argument('--p_cutoff', help='p cutoff for index SNP. defaut: 1e-9. p_nominal', default=1e-9, type=float)
    parser.add_argument('--snp_col', help='num of col of SNP id (default: 2 )', default=2, type=int)
    parser.add_argument('--pval_col', help='num of col of pval (default: 7 )', default=7, type=int)
    ...
    snp_col_index = int(args.snp_col) - 1
    p_col_index = int(args.pval_col) - 1
    r2_dict = make_r2_dict(in_ld_addr)

    junct_region(in_addr, out_addr, r2_dict, p_cutoff, r2_cutoff, snp_col_index, p_col_index)
