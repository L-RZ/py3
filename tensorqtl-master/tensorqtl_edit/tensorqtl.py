...
def main():
    ...
    if args.mode.startswith('cis'):
        if args.mode=='cis':
            res_df = cis_ase.map_cis(genotype_df, variant_df, phenotype_df, phenotype_pos_df, covariates_df,
            ...
            if not args.load_split:
                ...
                for chrom in pr.chrs:
                    ...
                    variant_df = pr.bim.set_index('snp')[['chrom', 'pos']]
                    chr_df = cis_ase.map_nominal(genotype_df, variant_df[variant_df['chrom']==chrom],
