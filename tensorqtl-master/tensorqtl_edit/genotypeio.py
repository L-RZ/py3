...
class InputGeneratorCis(object):
    ...
    def __init__(self, genotype_df, variant_df, phenotype_df, phenotype_pos_df, group_s=None, window=1000000):
        ...
        m1 = np.all(phenotype_df.values == phenotype_df.values[:,[0]], 1)
        phe_values_matrix = np.array(phenotype_df.values)  # check for constant phenotypes after remove missing value (-1)
        phe_values_matrix[phe_values_matrix == -1] = np.nan
        m2 = np.nanstd(phe_values_matrix, 1) == 0
        m = m1 | m2
        if m.any():
        ...
        # check for samples with non-missing phenotypes' value (missing value: -1 for ASE z-score analysis)
        # remove phenotypes with only 1 or 2 value
        m = np.sum(self.phenotype_df.values != -1, 1) < 3
        if m.any():
            print('    ** dropping {} phenotypes non-missing value < 3'.format(np.sum(m)))
            self.phenotype_df = self.phenotype_df.loc[~m]
            self.phenotype_pos_df = self.phenotype_pos_df.loc[~m]
